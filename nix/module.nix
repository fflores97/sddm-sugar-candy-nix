inputs: {
  ...
  theme-conf-file =
    pkgs.writeText "sddm-sugar-candy-nix.conf" (mkThemeConf
      cfg.settings);
  ...
  options.services.displayManager.sddm.sugarCandyNix = {
    ...
    settings = {
      Background = mkOption {
        ...
        description = ''
          Set the theme background. Must be a path.
      ...
      BlurRadius = mkOption {
        default = 100;
        example = 0;
        description = "Set the strength of the blur effect. Anything above 100 is pretty strong and might slow down the rendering time. 0 is like setting false for any blur.";
      ...
      HaveFormBackground = mkOption {
        ...
        example = true;
        description = ''
          Have a full opacity background color behind the form that takes slightly more than 1/3 of screen estate.

          If `PartialBlur` is set to true then `HaveFormBackground` will trigger the BackgroundColor of the form element to be partially transparent and blend with the blur.
        '';
        type = types.bool;
      };
      ...
      FormPosition = mkOption {
        default = "center";
        example = "left";
        description = "Position of the form which takes roughly 1/3 of screen estate. Can be left, center or right.";
      ...
      OverrideLoginButtonTextColor = mkOption {
        ...
        example = "#ffffff";
        description = ''
          The text of the login button may become difficult to read depending on your color choices.

          Use this option to set it independently for legibility.
        '';
        type = types.str;
      };
      ...
      InterfaceShadowSize = mkOption {
        ...
        example = 3;
        description = ''
          Integer used as multiplier. Size of the shadow behind the user and session selection background.

          Decrease or increase if it looks bad on your background. Initial render can be slow for values above 5-7.
        '';
        type = types.ints.unsigned;
      };
      ...
      InterfaceShadowOpacity = mkOption {
        ...
        example = 0.1;
        description = ''
          Float between 0 and 1. Alpha channel of the shadow behind the user and session selection background.
      ...
      Font = mkOption {
        default = "Noto Sans";
        example = "Fira Code";
        description = "Select a custom font. Must be installed globally in your Nix configuration.";
        type = types.nonEmptyStr;
      };
      ...
      ForceLastUser = mkOption {
        default = true;
        example = false;
        description = "Have the last successfully logged in user appear automatically in the username field.";
        type = types.bool;
      };
      ...
      ForcePasswordFocus = mkOption {
        default = true;
        example = false;
        description = "Give automatic focus to the password field. Together with forceLastUser this makes for the fastest login experience.";
        type = types.bool;
      };
      ...
      ForceHideCompletePassword = mkOption {
        default = false;
        example = true;
        description = "If you don't like to see any character at all not even while being entered set this to true.";
        type = types.bool;
      };
      ...
      ForceHideSystemButtons = mkOption {
        default = false;
        example = true;
        description = "Completely disable and hide any power buttons on the greeter.";
        type = types.bool;
      };
